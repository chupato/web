...
function fuzzyFast(str, pattern, s, p, score, bonus, deepness) {
  ...
  if (lowerC === pattern[p]) {
    ...
    let altMatch = (deepness > 0)
      ? fuzzyFast(str, pattern, s+1, p, score, 0, deepness - 1)
      : 0;
    ...
    score += 1 + bonus;
    bonus += 5;
    const match = fuzzyFast(str, pattern, s+1, p+1, score, bonus, deepness);
    return (altMatch < match) ? match : altMatch;
  } else {
    bonus = c === '-' ? 3 : 0;
    return fuzzyFast(str, pattern, s+1, p, score, bonus, deepness);
  }
}

function _ArrayToHTML(arr, baseStr) {
...
function matchFuzzy(block, pattern) {
  var bestMatch = fuzzy(block.normalized, pattern, 0, 0, 0, 8, [], 6);
  block.score += bestMatch.score;
  block.HTML = _ArrayToHTML(bestMatch.matched, block.str);
